...
export function ManualSeating({ schoolYearId, onBack, initialAssignments = [], onPreview, onConfirmed }: Props): JSX.Element {
  ...
  const confirm = (): void => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(occurredOn)) { setMessage("자리배치 날짜를 YYYY-MM-DD 형식으로 입력하세요."); return; }
    setMessage("");
    setShowConfirmDialog(true);
  };
  const saveConfirmed = (): void => {
    const currentRows = rows();
    setIsSaving(true);
    void window.appApi.seating.confirm(schoolYearId, layoutId, students, desks, currentRows, semester, occurredOn).then((sequence) => { const savedMessage = `${semester}학기 ${sequence}차 자리배치로 확정 저장했습니다.`; setMessage(savedMessage); setToast(savedMessage); setDirty(false); setShowConfirmDialog(false); onConfirmed?.(currentRows); onPreview(currentRows); }).catch((reason: unknown) => setMessage(reason instanceof Error ? reason.message : "자리배치 확정에 실패했습니다.")).finally(() => setIsSaving(false));
  };
  ...
  return <main className="app-shell">
    ...
    {showConfirmDialog && <ConfirmDialog title="이 배치를 확정 저장할까요?" confirmLabel={isSaving ? "저장 중…" : "확정 저장"} tone="primary" onCancel={() => setShowConfirmDialog(false)} onConfirm={saveConfirmed}>현재 배치 {students.length}명을 {semester}학기 {occurredOn} 날짜의 자리 기록으로 저장합니다. 확정 후 기록 화면과 다음 추천에 반영됩니다.</ConfirmDialog>}
